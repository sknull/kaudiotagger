...
enum class ID3v22KFrame(
    ...
    companion object {
        fun fromId(id: String): ID3v22KFrame? = ID3v22KFrame.entries.find { e -> id == e.id }

        fun fromFieldKey(fieldKey: KFieldKey): ID3v22KFrame? = ID3v22KFrame.entries.find { e -> fieldKey == e.fieldKey }
    }

    override fun commonFrames(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.isCommon }

    override fun binaryFrames(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.isBinary }

    override fun multipleFrames(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.isMultipleAllowed }

    override fun supprtedFrames(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.isSupported }

    override fun extensionFrames(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.isExtension }

    override fun discardIfFileAltered(): List<ID3v22KFrame> = ID3v22KFrame.entries.filter { e -> e.shouldBeDiscardedOnChange }
}