...
class ID3v24EncodingFlags(
    ...
    companion object {
         ...
         */
        val MASK_GROUPING_IDENTITY: Byte = FileConstants.BIT6

        /**
         * Frame is compressed
         */
        val MASK_COMPRESSION: Byte = FileConstants.BIT3

        /**
         * Frame is encrypted
         */
        val MASK_ENCRYPTION: Byte = FileConstants.BIT2

        /**
         * Unsynchronisation
         */
        val MASK_FRAME_UNSYNCHRONIZATION: Byte = FileConstants.BIT1

        /**
         * Length
         */
        val MASK_DATA_LENGTH_INDICATOR: Byte = FileConstants.BIT0
    }


    fun logEnabledFlags() {
        if (isNonStandardFlags()) {
            id3v24Frame!!.log.warn(
                "${id3v24Frame.identifier}:Unknown Encoding Flags:${flags.toHexString()}"
            )
        }
        if (isCompression()) {
            id3v24Frame!!.log.warn(
                ErrorMessage.MP3_FRAME_IS_COMPRESSED.getMsg(
                    id3v24Frame.identifier
                )
            )
        }
