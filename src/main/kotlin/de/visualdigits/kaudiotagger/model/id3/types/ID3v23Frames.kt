...
enum class ID3v23Frames(
    ...
    companion object {
        ...
        fun fromId(id: String?): ID3v23Frames? = entries.find { e -> id == e.id }

        fun fromFieldKey(fieldKey: GenericFieldKey): ID3v23Frames? = entries.find { e -> fieldKey == e.genericFieldKey }

        fun commonFrames(): List<ID3v23Frames> = entries.filter { e -> e.isCommon }

        fun binaryFrames(): List<ID3v23Frames> = entries.filter { e -> e.isBinary }

        fun multipleFrames(): List<ID3v23Frames> = entries.filter { e -> e.isMultipleAllowed }

        fun supprtedFrames(): List<ID3v23Frames> = entries.filter { e -> e.isSupported }

        fun extensionFrames(): List<ID3v23Frames> = entries.filter { e -> e.isExtension }

        fun discardIfFileAltered(): List<ID3v23Frames> = entries.filter { e -> e.isDiscardedIfFileAltered }
