...
enum class ID3v23KFrame(
    ...
    companion object {
        ...
        fun contains(id: String?): Boolean = ID3v23KFrame.entries.any { e -> id == e.id }
        ...
        fun commonFrames(): List<ID3v23KFrame> = entries.filter { e -> e.isCommon }

        fun binaryFrames(): List<ID3v23KFrame> = entries.filter { e -> e.isBinary }

        fun multipleFrames(): List<ID3v23KFrame> = entries.filter { e -> e.isMultipleAllowed }

        fun supprtedFrames(): List<ID3v23KFrame> = entries.filter { e -> e.isSupported }

        fun extensionFrames(): List<ID3v23KFrame> = entries.filter { e -> e.isExtension }

        fun discardIfFileAltered(): List<ID3v23KFrame> = entries.filter { e -> e.isDiscardedIfFileAltered }
